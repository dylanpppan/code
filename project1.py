...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    loss = np.maximum(1 - y * labels, np.zeros(len(labels)))
...
def pegasos_single_step_update(
    ...
    if label * (np.dot(feature_vector, current_theta) + current_theta_0) <= 1:
        return ((coeff * current_theta) + (eta * label * feature_vector),
                (current_theta_0) + (eta * label))
    return (coeff * current_theta, current_theta_0)
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    # Your code here
    (NumberOfSample, features_v) = feature_matrix.shape
    theta = np.zeros(features_v)
    theta_0 = 0
    count = 0
    
    for t in range(T):
        for i in get_order(NumberOfSample):
            count = count + 1
            eta = 1 / np.sqrt(count)
...
def bag_of_words_2(texts, stopwords=None):
    """
    Inputs a list of string reviews
    Returns a dictionary of unique unigrams occurring over the input
...
def extract_bow_feature_vectors_2(reviews, dictionary,use_count=False):
    """
    Inputs a list of string reviews
    Inputs the dictionary of words as given by bag_of_words
    Returns the bag-of-words feature matrix representation of the data.
